...
func executeTaskSubmissionWith(
	...
	task.PrivateData.BillingContext = &model.TaskBillingContext{
		...
		PerCallBilling: common.StringsContains(constant.TaskPricePatches, relayInfo.OriginModelName) ||
			...
			(relayInfo.TieredBillingSnapshot != nil && !relayInfo.TieredBillingSnapshot.TaskUsageBilling),
...
func presentTaskSubmission(c *gin.Context, outcome *taskSubmissionOutcome) {
	...
	taskRelayInfo := outcome.RelayInfo.TaskRelayInfo
	if taskRelayInfo != nil && len(taskRelayInfo.PendingResponse) > 0 {
		statusCode := taskRelayInfo.PendingResponseStatusCode
		if statusCode == 0 {
			statusCode = http.StatusOK
		}
		diagnostics.present(outcome.Task, "legacy_pending_response")
		c.Data(statusCode, "application/json", taskRelayInfo.PendingResponse)
		return
	}
