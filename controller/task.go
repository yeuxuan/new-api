...
func GetTask(c *gin.Context) {
	...
	if task.Platform == constant.TaskPlatform(strconv.Itoa(constant.ChannelTypeTMLabSeedance)) {
		adaptor := relay.GetTaskAdaptor(task.Platform)
		converter, ok := adaptor.(relaychannel.NativeTaskConverter)
		if !ok {
			videoProxyError(c, http.StatusInternalServerError, "server_error", "Task adapter unavailable")
			return
		}
		body, convertErr := converter.ConvertToNativeTask(task)
		if convertErr != nil {
			videoProxyError(c, http.StatusInternalServerError, "server_error", "Failed to render task")
			return
		}
