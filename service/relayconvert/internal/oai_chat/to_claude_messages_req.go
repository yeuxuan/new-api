				...
				default:
					...
					if mediaMessage.Type == dto.ContentTypeFile {
						if file := mediaMessage.GetFile(); file != nil {
							if dotIndex := strings.LastIndex(file.FileName, "."); dotIndex >= 0 && dotIndex < len(file.FileName)-1 {
								mimeType := relaymedia.ResolveMimeTypeByExtension(file.FileName[dotIndex+1:])
