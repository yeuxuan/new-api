...
func (*CreemAdaptor) RequestPay(c *gin.Context, req *CreemPayRequest) {
	...
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Creem 充值订单创建成功 user_id=%d trade_no=%s product_id=%s product_name=%q usd_quota=%.4f internal_quota=%d money=%.2f", id, referenceId, selectedProduct.ProductId, selectedProduct.Name, selectedProduct.Quota, topUp.Amount, selectedProduct.Price))
...
func handleCheckoutCompleted(c *gin.Context, event *CreemWebhookEvent) {
	...
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Creem Webhook order.type=%q，继续处理充值 trade_no=%s creem_order_id=%s", event.Object.Order.Type, referenceId, event.Object.Order.Id))
	if event.Object.Order.Type == "recurring" {
		logger.LogInfo(c.Request.Context(), fmt.Sprintf("Creem 收到 recurring 类型订单但未命中订阅表，跳过处理 trade_no=%s creem_order_id=%s", referenceId, event.Object.Order.Id))
		c.Status(http.StatusOK)
		return
	}
	...
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Creem 充值成功 trade_no=%s creem_order_id=%s internal_quota=%d money=%.2f client_ip=%s", referenceId, event.Object.Order.Id, topUp.Amount, topUp.Money, c.ClientIP()))
	c.Status(http.StatusOK)
}
...
func genCreemLink(ctx context.Context, referenceId string, product *CreemProduct, email string, username string) (string, error) {
	...
	if email != "" {
		requestData.Customer = &CreemCheckoutCustomer{Email: email}
	}
