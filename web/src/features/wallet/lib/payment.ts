...
export function getMinTopupAmount(
  ...
  const selectedMethod = paymentType
    ? topupInfo.pay_methods?.find((method) => method.type === paymentType)
    : topupInfo.pay_methods?.[0]
  const selectedMinTopup = Number(selectedMethod?.min_topup)
  if (Number.isFinite(selectedMinTopup) && selectedMinTopup > 0) {
    return selectedMinTopup
  }
