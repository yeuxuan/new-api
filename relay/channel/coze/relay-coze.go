...
func checkIfChatComplete(a *Adaptor, c *gin.Context, info *relaycommon.RelayInfo) (error, bool) {
	...
	resp, err := doRequest(c, req, info, "poll") // 调用 doRequest
	if err != nil {
		return err, false
	}
...
func getChatDetail(a *Adaptor, c *gin.Context, info *relaycommon.RelayInfo) (*http.Response, error) {
	...
	resp, err := doRequest(c, req, info, "detail")
	if err != nil {
...
func doRequest(c *gin.Context, req *http.Request, info *relaycommon.RelayInfo, stage string) (*http.Response, error) {
	client, err := service.GetHttpClientWithProxySettings(info.ChannelSetting.Proxy, info.ChannelSetting)
	if err != nil {
		return nil, fmt.Errorf("new proxy http client failed: %w", err)
	}
	operation := service.CaptureConversationAuxiliaryRequest(c, info, "coze_"+stage, req)
