...
describe('payment type classification', () => {
  test('keeps Waffo and Waffo Pancake on their dedicated flows', () => {
    ...
    assert.equal(isIPayNowPayment(PAYMENT_TYPES.IPAYNOW), true)
  })
})

describe('minimum top-up selection', () => {
  test('uses the selected provider minimum instead of the first method', () => {
    const topupInfo = {
      pay_methods: [
        { name: 'iPayNow', type: PAYMENT_TYPES.IPAYNOW, min_topup: 50 },
        { name: 'Stripe', type: PAYMENT_TYPES.STRIPE, min_topup: 10 },
      ],
    }

    assert.equal(
      getMinTopupAmount(topupInfo as TopupInfo, PAYMENT_TYPES.STRIPE),
      10
    )
    assert.equal(
      getMinTopupAmount(topupInfo as TopupInfo, PAYMENT_TYPES.IPAYNOW),
      50
    )
  })
})
