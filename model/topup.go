...
const (
	...
	PaymentProviderIPayNow      = "ipaynow"
...
// RechargeIPayNow atomically completes an iPayNow order. Historical pending
// orders may not have payment_provider populated, but payment_method is always
// present and remains the authoritative cross-gateway guard for those rows.
func RechargeIPayNow(tradeNo string, callerIp string) (alreadyDone bool, err error) {
	if tradeNo == "" {
		return false, errors.New("未提供支付单号")
	}
	...
	err = DB.Transaction(func(tx *gorm.DB) error {
		...
		if topUp.PaymentMethod != PaymentMethodIPayNow ||
			(topUp.PaymentProvider != "" && topUp.PaymentProvider != PaymentProviderIPayNow) {
			return ErrPaymentMethodMismatch
		}
		if topUp.Status == common.TopUpStatusSuccess {
			alreadyDone = true
	...
	if err != nil {
		if !errors.Is(err, ErrTopUpNotFound) && !errors.Is(err, ErrPaymentMethodMismatch) && !errors.Is(err, ErrTopUpStatusInvalid) {
			common.SysError("ipaynow topup failed: " + err.Error())
		}
		return false, err
	}
	if alreadyDone {
	...
	syncCreditUserQuotaCache(topUp.UserId, quotaToAdd, "ipaynow topup")
	RecordTopupLogWithQuota(
		topUp.UserId,
		fmt.Sprintf("使用聚合动态码充值成功，充值金额: %v，支付金额：%.2f", logger.LogQuota(quotaToAdd), topUp.Money),
		quotaToAdd,
		callerIp,
		topUp.PaymentMethod,
		PaymentProviderIPayNow,
	)
	return false, nil
}
...
func Recharge(referenceId string, customerId string, callerIp string) (err error) {
	...
	RecordTopupLogWithQuota(topUp.UserId, fmt.Sprintf("使用在线充值成功，充值金额: %v，支付金额：%d", logger.FormatQuota(quota), topUp.Amount), quota, callerIp, topUp.PaymentMethod, PaymentMethodStripe)
...
func ManualCompleteTopUp(tradeNo string, callerIp string) error {
	...
	err := DB.Transaction(func(tx *gorm.DB) error {
		...
		// - Creem 订单：Amount 在创建订单时已预计算为内部额度（Quota × QuotaPerUnit），直接使用
	...
	RecordTopupLogWithQuota(userId, fmt.Sprintf("管理员补单成功，充值金额: %v，支付金额：%f", logger.FormatQuota(quotaToAdd), payMoney), quotaToAdd, callerIp, paymentMethod, "admin")
	return nil
}
func RechargeCreem(referenceId string, customerEmail string, customerName string, callerIp string) (err error) {
	...
	err = DB.Transaction(func(tx *gorm.DB) error {
		...
		// Creem Amount 在创建订单时已预计算为内部额度（Quota × QuotaPerUnit），此处直接使用
	...
	RecordTopupLogWithQuota(topUp.UserId, fmt.Sprintf("使用Creem充值成功，充值额度: %v，支付金额：%.2f", logger.FormatQuota(int(quota)), topUp.Money), int(quota), callerIp, topUp.PaymentMethod, PaymentMethodCreem)
