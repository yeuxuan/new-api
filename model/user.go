...
func GetAllUsers(pageInfo *common.PageInfo) (users []*User, total int64, err error) {
	...
	err = tx.Unscoped().Order(pageInfo.GetOrder()).Limit(pageInfo.GetPageSize()).Offset(pageInfo.GetStartIdx()).Omit("password", "access_token").Find(&users).Error
	if err != nil {
		tx.Rollback()
		return nil, 0, err
	}
...
func SearchUsers(keyword string, group string, role *int, status *int, pageInfo *common.PageInfo) ([]*User, int64, error) {
	...
	// 获取分页数据
	err = query.Omit("password", "access_token").Order(pageInfo.GetOrder()).Limit(num).Offset(startIdx).Find(&users).Error
	if err != nil {
		tx.Rollback()
		return nil, 0, err
	}
