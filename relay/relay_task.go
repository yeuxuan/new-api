...
func RelayTaskSubmit(c *gin.Context, info *relaycommon.RelayInfo) (*TaskSubmitResult, *dto.TaskError) {
	...
	if useTiered {
		if !exists {
			return nil, service.TaskErrorWrapper(fmt.Errorf("task model %s has no billing expression", modelName), "model_price_error", http.StatusBadRequest)
		}
		requestInput, inputErr := helper.ResolveIncomingBillingExprRequestInput(c, info)
		if inputErr != nil {
			return nil, service.TaskErrorWrapper(inputErr, "model_price_error", http.StatusBadRequest)
		}
		...
		if !taskUsageBilling {
			// This fork historically allowed request-aware task expressions using
			// token-expression units. Preserve those stored contracts while new
			// u(...) task expressions use direct USD task pricing.
			quotaBeforeGroup /= 1_000_000
		}
		quota, quotaErr := common.QuotaRoundStrict(quotaBeforeGroup * groupRatioInfo.GroupRatio)
		if quotaErr != nil {
			return nil, service.TaskErrorWrapper(quotaErr, "model_price_error", http.StatusBadRequest)
		}
		priceData = types.PriceData{Quota: quota, QuotaToPreConsume: quota, GroupRatioInfo: groupRatioInfo}
		info.TieredBillingSnapshot = &billingexpr.BillingSnapshot{BillingMode: billing_setting.BillingModeTieredExpr, ModelName: modelName, ExprString: exprStr, ExprHash: billingexpr.ExprHashString(exprStr), GroupRatio: groupRatioInfo.GroupRatio, EstimatedQuotaBeforeGroup: quotaBeforeGroup, EstimatedQuotaAfterGroup: quota, EstimatedTier: trace.MatchedTier, QuotaPerUnit: common.QuotaPerUnit, ExprVersion: billingexpr.ExprVersion(exprStr), TaskUsageBilling: taskUsageBilling, UsageFacts: facts}
