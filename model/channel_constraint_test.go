...
func TestFilterCandidateIDs(t *testing.T) {
	...
	tmlab := &Channel{Id: 900006, Type: constant.ChannelTypeTMLabSeedance, Status: common.ChannelStatusEnabled}
	...
	nativeTaskPathFilter := dto.ChannelFilter{Kind: dto.FilterRequestPath, RequestPath: "/v1/tasks"}
	...
	tests := []struct {
		...
		{
			name:      "native task path keeps only TMLab",
			ids:       []int{900001, 900003, 900006, 999999},
			modelName: "seedance-2.0-fast",
			filters:   []dto.ChannelFilter{nativeTaskPathFilter},
			wantKept:  []int{900006},
		},
		{
...
func TestChannelSatisfiesFilters(t *testing.T) {
	...
	tmlab := &Channel{Id: 4, Type: constant.ChannelTypeTMLabSeedance}
	...
	nativeTaskFilter := []dto.ChannelFilter{{
		Kind:        dto.FilterRequestPath,
		RequestPath: "/v1/tasks",
	}}
	ok, kind = ChannelSatisfiesFilters(tmlab, "seedance-2.0-fast", nativeTaskFilter)
	require.True(t, ok)
	assert.Equal(t, dto.ChannelFilterKind(""), kind)

	ok, kind = ChannelSatisfiesFilters(alpha, "seedance-2.0-fast", nativeTaskFilter)
	assert.False(t, ok)
	assert.Equal(t, dto.FilterRequestPath, kind)
}
